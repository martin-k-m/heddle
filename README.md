...
**heddle does not run yet.**

heddle is written in twill, in `.tw` files, and it uses `mode systems`, the
...
there is no binary, no sampler you can start, and every code block below
describes behaviour heddle is written to have rather than behaviour it has.

Writing a real inference library against the subset is how you find out what the
subset is missing. What it is missing is written down in
[`docs/needs.md`](docs/needs.md): one entry per feature, naming the file and the
function that needs it and what heddle does in the meantime. That list is the
